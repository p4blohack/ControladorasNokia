...
# Lock para escritura segura en listas (opcional)
list_lock = Lock()
...
def tarea_equipo(equipo, client, password, output_file, output_file_comandos):
    ip = equipo['Ip']
    nombre = equipo['Nombre']
    jerarquia = equipo['Jerarquia']
    try:
        tiene_fallas, fallas, resultado_mant = procesar_equipo_completo(
            client, ip, nombre, password, output_file, output_file_comandos, jerarquia
        )
        ...
        # Registrar fallo global de conexión
        with list_lock:
            errores_conexion.append({'Nombre': nombre, 'Ip': ip, 'Jerarquia': jerarquia, 'Razon': str(e)})
    return
...
with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
    ...
    for equipo in equipos:
        futures.append(
            executor.submit(tarea_equipo, equipo, client, password, output_file, output_file_comandos)
        )
